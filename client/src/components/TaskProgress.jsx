...
const TaskProgress = ({ analytics: analyticsProp, loading: loadingProp }) => {
  const context = useTask ? useTask() : {};
  const analytics = analyticsProp || context.analytics;
  const loading = typeof loadingProp === 'boolean' ? loadingProp : context.loading;
  const fetchAnalytics = context.fetchAnalytics;
  const [isRefreshing, setIsRefreshing] = useState(false);

  useEffect(() => {
    if (!analyticsProp && !analytics && fetchAnalytics) {
      fetchAnalytics();
    }
  }, [analytics, analyticsProp, fetchAnalytics]);
  ...
  if (analytics.has_active_cycle === false) {
    return (
  ...
  // Format dates
  const startDate = analytics.cycle_start_date ? new Date(analytics.cycle_start_date).toLocaleDateString() : '';
  const endDate = analytics.cycle_end_date ? new Date(analytics.cycle_end_date).toLocaleDateString() : '';

  return (
    <div className="bg-white dark:bg-secondary-800 rounded-lg shadow-card p-6 transition-colors duration-200">
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-lg font-semibold text-secondary-900 dark:text-white">Current Progress</h3>
        {fetchAnalytics && !analyticsProp && (
          <button 
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-3 text-center transition-colors duration-200">
          <div className="text-3xl font-bold text-primary-600 dark:text-primary-400">{analytics.days_elapsed ?? analytics.days_completed}</div>
        ...
        <div className="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-3 text-center transition-colors duration-200">
          <div className="text-3xl font-bold text-primary-600 dark:text-primary-400">{Math.round(analytics.overall_completion_rate ?? analytics.completion_rate ?? 0)}%</div>
      ...
      <div className="mb-2">
        <div className="flex justify-between text-sm mb-1 text-secondary-900 dark:text-white">
          ...
          <span>{Math.round(analytics.progress_percentage ?? analytics.completion_rate ?? 0)}%</span>
        </div>
        <div className="progress-bar bg-gray-200 dark:bg-secondary-700 rounded-full h-2.5">
          <div 
            ...
            style={{ width: `${analytics.progress_percentage ?? analytics.completion_rate ?? 0}%` }}
          ></div>
        </div>
      </div>
              ...
              <div className="mt-2">
                ...
                <div className="progress-bar bg-gray-200 dark:bg-secondary-600 rounded-full h-2">
                  <div 
                    ...
                    style={{ 
                      width: `${(task.completion_rate ?? 0) * 100}%`,
                      backgroundColor: (task.completion_rate ?? 0) >= 0.7 ? '#22c55e' : '#0ea5e9'
                    }}
                  ></div>
                </div>
              </div>
